            ...
            if unit_split and unit_col and unit_col in df_in.columns:
                ...
                with ZipFile(zip_bytes, 'w') as zf:
                    for u in units:
                        ...
                        style_and_save_excel(
                            sub_final, sub_excel_io,
                            header_fill=header_color,
                            subtotal_fill=subtotal_color,
                            grand_fill=grand_color,
                            bold_cols=bold_cols
                        )
                ...
                zip_bytes.seek(0)
                st.download_button(
                    "Download ZIP (one file per unit)",
                    data=zip_bytes,
                    file_name=f"TakeoffNSW_units_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
                )
            else:
                st.download_button(
                    "Download styled Excel",
                    data=excel_io,
                    file_name=f"TakeoffNSW_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                )
                st.download_button(
                    "Download CSV",
                    data=csv_io,
                    file_name=f"TakeoffNSW_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                )
